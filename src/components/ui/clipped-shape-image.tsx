...
const ClippedShapeGallery = React.forwardRef<HTMLElement, ClippedShapeGalleryProps>(
  ({ mediaItems, className, ...props }, ref) => {
    ...
    const defaultMediaItems: MediaItem[] = [
      {
        ...
        alt: 'Premium Barber Setup',
        clipId: 'clip-another1',
        type: 'image',
      },
      {
        src: 'https://images.unsplash.com/photo-1621605815971-8bcaf47d6cb0?q=80&w=600&auto=format&fit=crop',
        alt: 'Precision Straight Razor',
    ...
    return (
      <>
        <svg className="absolute -top-[999px] -left-[999px] w-0 h-0 pointer-events-none">
          <defs>
        ...
        <section
          ref={ref}
          className={`w-full max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8 p-4 md:p-8 ${className || ''}`}
          {...props}
        >
          {itemsToRender.map((item, index) => (
            <figure key={index} style={{ clipPath: `url(#${item.clipId})` }} className="relative group overflow-hidden">
              <div className="absolute inset-0 bg-emerald-500/20 mix-blend-overlay z-10 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none" />
              {item.type === 'image' ? (
                <img
                  src={item.src}
                  alt={item.alt}
                  className="transition-all duration-700 aspect-[4/5] min-h-[400px] md:min-h-full align-bottom object-cover group-hover:scale-110 w-full grayscale group-hover:grayscale-0"
                />
              ) : (
                <video
                  autoPlay
